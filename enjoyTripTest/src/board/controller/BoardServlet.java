...
public class BoardServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// logout 후 뒤로 가기 클릭하면 캐쉬된 boardMain 이 보인다. 그 뒤 totalCnt 가져오는 부분에서 ajax 로 요청하다가
		// login.jsp 페이지 결과 받는 오류 발생
...
//            default : notValidUrl();
	...
	private void boardInsert(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("request.getParameter(\"board_title\") " + request.getParameter("title"));
		
		System.out.println(userDto.getUserName());
		boardDto.setUserSeq(userDto.getUserId());
		boardDto.setBoard_writername(userDto.getUserName());
	...
	private void boardDetail(HttpServletRequest request, HttpServletResponse response)
		...
		int userSeq = userDto.getUserId();
		System.out.println("boardDetail: " + userSeq);
