...
window.onload = function() {
	...
	document.querySelector("#btnStudentDetail").onclick = async function() {
		// 비동기 요청
		try {
			let response = await fetch('/mydb/studentDetail/1');
	...
	document.querySelector("#btnStudentList").onclick = async function() {
		try {
			let response = await fetch('/mydb/studentList');
			let data = await response.json();
			console.log(data);
		} catch (error) {
			console.error(error);
	...
	document.querySelector("#btnStudentDelete").onclick = async function() {
		...
		try {
			let response = await fetch('/mydb/studentDelete/1');
			let data = await response.json();
			console.log(data);
		} catch (error) {
			console.error(error);
	...
	document.querySelector("#btnStudentUpdate").onclick = async function() {
		...
		try {
			let response = await fetch('/mydb/studentUpdate', fetchOptions);
			let data = await response.json();
			console.log(data);
		} catch (error) {
			console.error(error);
	...
	document.querySelector("#btnStudentInsertJSON").onclick = async function() {
		...
		try {
			let response = await fetch('/mydb/studentInsertJSON', fetchOptions);
			let data = await response.json();
			console.log(data);
		} catch (error) {
			console.error(error);
